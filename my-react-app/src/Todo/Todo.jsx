...
import { addTodo, editTodo, updateTodo, deleteTodo, toggleComplete } from "./Reducer";
            ...
            <div className="container" style={{ width: "100%", height: "auto", display: "flex", justifyContent: "center", alignItems: "center", textAlign: 'center', color: 'black', padding: "50px" }}>
                <div style={{ width: "700px", height: "auto", padding: '30px', backgroundColor: 'rgba(255, 255, 255, 0.5)', backdropFilter: 'blur(5px)', borderRadius: "50px" }}>
                    <div className="container">
                        ...
                        <MDBBtn onClick={addHandle} color="black" className="m-3"><MDBIcon fab icon='add' /></MDBBtn>
                    </div>
                                    ...
                                    <>
                                        <div className="container" style={{ border: "2px solid rgb(51, 51, 51)", margin: '3px', boxShadow: "1px 1px 2px 2px rgb(128, 128, 128)" }}>
                                            <div style={{ textAlign: "right" }}>
                                                <MdOutlineDeleteForever onClick={() => dispatch(deleteTodo(Data.id))} size={25} color="rgb(153, 0, 0)" />
                                            </div>
                                            <p className="text-break" style={{ textDecoration: Data.completed ? 'line-through' : 'none' }}>
                                                {Data.task}
                                            </p>
                                            <div className="m-3" style={{ textAlign: 'center' }}>
                                                <FaRegEdit onClick={() => dispatch(editTodo(Data.id))} size={20} color="rgb(153, 0, 0)" />
                                                <span style={{ marginLeft: '10px' }} onClick={() => toggleCompletion(Data.id)}>
                                                    {Data.completed ? <MdCheckBox size={25} color="green" /> : <MdCheckBoxOutlineBlank size={25} color="blue" />}
                                                </span>
                                            </div>
                                        </div>
                                    </>
